...
SpectrumAnalyzerComponent::SpectrumAnalyzerComponent(float hostSr)
...
void SpectrumAnalyzerComponent::analysisThreadMain()
{
    ...
    while (running.load(std::memory_order_relaxed))
    {
        ...
        while (ring.pop(p))
        {
            // nephos puts mono audio into the fifo
            auto m = p; // 0.5f * (p->first + p->second);
...
{
    auto bg = juce::Colours::black;
    auto curveCol = juce::Colours::white;
    auto peakCol = curveCol.darker(0.4f);
    auto frameCol = juce::Colours::grey;
    ...
    auto labelCol = juce::Colours::yellow;
    auto insetBg = bg.darker(0.4f);
