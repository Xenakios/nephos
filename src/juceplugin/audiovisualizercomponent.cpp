...
void XenAudioVisualizerComponent::drawNextLineOfSpectrogram()
{
    ...
    const bool measure_level_range = false;
    if (measure_level_range)
    {
        auto lo = 1000.0f, hi = -1000.0f;
        for (int i = 0; i < fftSize / 2; ++i)
        {
            auto db = juce::Decibels::gainToDecibels(localFFTData[(size_t)i]);
            lo = juce::jmin(lo, db);
            hi = juce::jmax(hi, db);
        }
        DBG(lo << " to " << hi);
